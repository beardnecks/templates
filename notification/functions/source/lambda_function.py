...
def lambda_handler(event, context):
    ...
    user = "beardnecks"
    repo = "suricata"
    pullrequests = requests.get('https://api.github.com/repos/' +user + '/' + repo +'/pulls', headers={"Authorization": "token " + token})
    if len(pullrequests.json()) > 0:
        sha = pullrequests.json()[0]["head"]["sha"]
    else:
        # Only looks at push to master bran
        pushesToMaster = requests.get('https://api.github.com/repos/'+user+'/'+repo+'/commits/master', headers={"Authorization": "token " + token})
        sha = pushesToMaster.json()["sha"]
    ...
    if parsed_message['detail']['state'] == 'STARTED':
        r = requests.post('https://api.github.com/repos/' +user + '/' + repo +'/statuses/'+sha , json={
            "state": "pending",
