...
aws cloudformation create-stack --stack-name ${STACKNAME} --template-body file://prod-master.json \
  ...
  ParameterKey=PipelineName,ParameterValue="p-joachim-pipeline" \
  ParameterKey=SourceBucket,ParameterValue="t-martin-outputbucket" \
  ParameterKey=SourceBucketObjectKey,ParameterValue="beardnecks/suricata/prod/beardnecks_suricata.zip" \
  ParameterKey=OutputBucketName,ParameterValue="p-joachim-package" \
  ParameterKey=PipelineTemplateURL,ParameterValue="https://t-neckbeards-stackinit-s.s3-eu-west-1.amazonaws.com/${PREFIX}codePipeline/prod-pipeline.json" \
  ...
  ParameterKey=PackagingTemplateURL,ParameterValue="https://t-neckbeards-stackinit-s.s3-eu-west-1.amazonaws.com/${PREFIX}codeBuild/package.json" \
  ParameterKey=DockerBuildTemplateURL,ParameterValue="https://t-neckbeards-stackinit-s.s3-eu-west-1.amazonaws.com/${PREFIX}codeBuild/dockerBuild.json" \
  ParameterKey=DockerDeployTemplateURL,ParameterValue="https://t-neckbeards-stackinit-s.s3-eu-west-1.amazonaws.com/${PREFIX}codeBuild/dockerDeploy.json" \
